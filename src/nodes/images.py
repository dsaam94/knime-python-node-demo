...
class ImageProcessorNode:
    ...
    This node demos applying basic processing steps on a PNG image
    """
    ...
    def execute(self, exec_context: knext.ExecutionContext, input_table: knext.Table):
        ...
        # Remember to set exexution progress for end user to show at which point the execution is in the code
        # Use arbitrary value to show progress for end user. The range of values must be between 0.1 - 0.9
        exec_context.set_progress(0.1, "Processing images...")
        ...
        exec_context.set_progress(0.3, "Extracting Dimensions...")
        df["Image Dimensions"] = [list(i.size) for i in images]

        exec_context.set_progress(0.5, "Flipping Images...")
        df["Flipped"] = [ImageOps.flip(i) for i in images]

        exec_context.set_progress(0.8, "Blurring Images...")
        df["Blurred"] = [
