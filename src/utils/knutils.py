...
def is_smiles_value(column:knext.Column) -> bool:
    """
    Checks if column contains SMILES value
    @return: True if Column is of types SMILES value
    """
    return __is_type_x(column, knext.logical(ktchem.SmilesValue).logical_type)


def is_smiles_adapter_value(column:knext.Column) -> bool:
    """
    Checks if column contains SMILES Adapter value
    @return: True if Column is of type SMILES Adapter Value
    """
    return __is_type_x(column, knext.logical(ktchem.SmilesAdapterValue).logical_type)


def is_smiles(column:knext.Column) -> bool:
    """
    Checks if column contains SMILES value or SMILES adapter value
    @return: True if Column is SMILES
    """
    # LOGGER.warning(column.ktype)
    # LOGGER.warning(ktchem.SmilesValue)
    return boolean_or(is_smiles_value, is_smiles_adapter_value)(column)
